...
| HTTP smoke test (`dev` only) | `GET /health` anonymously, `GET /` anonymously (expect `401`), then `GET /` as the environment's end-to-end test client (credentials in the environment's Key Vault) on `<webapp>.azurewebsites.net`, up to 3 min | Sign in through the ALB with the environment's test user (credentials in Secrets Manager), then `GET /health` on `AWS_APP_URL`, up to 3 min |

The dev smoke test reports what it proved — one line per check with the path, the expected and observed status and the identity used, plus where the credentials came from — in the job log and in the run's step summary; a failure names the attempts made, the budget and the last step reached. The public hostname for staging/prod is unreachable from GitHub-hosted runners (private-only ingress), so the smoke test is dev-only; the control-plane assertions provide equivalent confidence without network access to the app. To use HTTP validation against staging/prod, provision self-hosted runners inside the private network and remove the environment condition on the smoke-test step.

Every successful deployment ends with a **"Confirm deployed version"** block, printed to the job log and to the run's step summary as a table, with the same fields in the same order on both clouds: environment and cloud, intended image reference, image reported by the control plane, digest actually running (Azure: registry manifest of the tag; AWS: the running task's `imageDigest`), the `APP_NAME` / `APP_ENV` / `IMAGE_TAG` runtime configuration as deployed, where it runs (resource group and Web App with the slot swapped, or region, cluster, service, task definition revision and controller with the CodeDeploy deployment id), the endpoints (public URL with `/health` and `/whois`, or "private ingress"), the timing (started, ready, validated, elapsed) and the control plane's own record of the deployment.

A failed rollout additionally triggers a **diagnostics step** on both clouds, so a failed deployment is diagnosable from the job log alone. On Azure it dumps, for the production slot and (staging/prod) the deployment slot, the Web App state and configuration, the per-instance state, the tail of the container log, and the recent activity log entries for the Web App (swap and restart operations with their status). On AWS it dumps the service deployments, recent service events, stopped-task stop codes and container exit codes — and, for blue/green, the recent CodeDeploy deployments with their error information.

**What the underlying infrastructure must provide:** each environment must expose the app's contract — the container listens on `PORT` (image default `8080`) as a **non-root** user, and serves `/health`. On Azure that means a Linux container Web App with `health_check_path = "/health"`, dev reachable publicly, and the deploy identity able to read the end-to-end test client's `e2e-*` secrets in the environment's Key Vault; on AWS an ECS Fargate service whose task definition, target groups and security groups speak port `8080`, with the CodeDeploy application/deployment group following the naming convention above for blue/green environments, and — for `dev` — the deploy role allowed to read the test user's secret (`auth/<app>-dev/*`) that the smoke test signs in with. Any infrastructure deviating from this needs matching changes to the deploy workflows.
